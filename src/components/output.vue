<template>
  <div class="card m-1">
            <div
              class="card-header"
              :class="value == 1 ? config.cardOnClass : config.cardOffClass"
            >
              <div class="d-flex flex-row justify-content-between flex-wrap">
                {{ config.cardText }} -
                {{ value == 1 ? config.cardOnText : config.cardOffText }}
                <button
                  @click="config.enableSound = !config.enableSound"
            ...
            <div class="card-body">
              <div>
                <div class="d-flex flex-row justify-content-around flex-wrap">
                  <button
                    :class="config.btnOnClass"
                    @click="out(pin, 't')"
                  >
                    {{ config.btnOnText }}
                  </button>
                  <button
                    :class="config.btnOffClass"
                    @click="out(pin, 't')"
                  >
                    {{ config.btnOffText }}
                  </button>
                  <button
                    :class="config.btnTglClass"
                    @click="out(pin, 't')"
                  >
                    {{ config.btnTglText }}
                  </button>
